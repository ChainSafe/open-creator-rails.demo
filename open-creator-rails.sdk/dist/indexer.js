...
import { INDEXER_ASSET_ENTITY_LIST_ORDER_BY, INDEXER_ASSET_ENTITY_LIST_ORDER_DIRECTION, INDEXER_SUBSCRIPTION_LIST_ORDER_BY, INDEXER_SUBSCRIPTION_LIST_ORDER_DIRECTION, } from "./types";
/** Matches `open-creator-rails.indexer` / `getAssetEntityId`. */
export function indexerAssetEntityId(chainId, assetAddress) {
    return `${chainId}_${assetAddress.toLowerCase()}`;
}
function assetContractAddressFromEntityId(assetEntityId) {
    const sep = assetEntityId.indexOf("_");
    if (sep === -1)
        throw new Error(`Invalid asset entity id: ${assetEntityId}`);
    return getAddress(assetEntityId.slice(sep + 1));
}
/**
 ...
 */
export function resolveOpenCreatorRailsIndexerGraphqlUrl(url) {
    const trimmed = url.trim().replace(/\/+$/, "");
    if (/\/v2\/graphql$/i.test(trimmed))
        return trimmed;
...
export function createSdkIndexer(indexerUrl, options) {
    const endpoint = resolveOpenCreatorRailsIndexerGraphqlUrl(indexerUrl);
    const { chainId } = options;
    const getSubscriptionBySubscriberId = async ({ assetAddress, subscriberHash: subscriberHashHex, }) => {
    ...
    `;
        ...
        return {
            ...
            payer: item.payer ? asAddress(item.payer) : ("0x" + "0".repeat(40)),
    ...
    `;
        const data = await graphql(endpoint, query, { address: assetAddress.toLowerCase() });
    ...
    const getAssetOwner = async ({ assetAddress }) => {
        const row = await getAsset({ assetAddress });
        return row?.owner ?? null;
    };
    const listSubscriptionsBySubscriberId = async ({ subscriberHash: subscriberHashHex, activeOnly, limit, offset, orderBy = INDEXER_SUBSCRIPTION_LIST_ORDER_BY, orderDirection = INDEXER_SUBSCRIPTION_LIST_ORDER_DIRECTION, }) => {
    ...
    const listSubscriptionsByUser = async ({ user, subscriberId, activeOnly, limit, offset, orderBy, orderDirection, }) => {
    ...
    const listAssetsByRegistry = async ({ registryAddress, limit, offset, orderBy = INDEXER_ASSET_ENTITY_LIST_ORDER_BY, orderDirection = INDEXER_ASSET_ENTITY_LIST_ORDER_DIRECTION, }) => {
        const query = `
