...
# ─── 3. Write .env.anvil ─────────────────────────────────────────────────────
# Keep Google Sheet URL across dev:local runs (not derived from seed).
EXISTING_SHEET_URL=""
EXISTING_SHEET_WRITE_URL=""
if [ -f "$ROOT_DIR/.env.anvil" ]; then
  EXISTING_SHEET_URL="$(grep '^VITE_DEMO_SERVICES_SHEET_URL=' "$ROOT_DIR/.env.anvil" 2>/dev/null | cut -d= -f2- || true)"
  EXISTING_SHEET_WRITE_URL="$(grep '^VITE_DEMO_SERVICES_SHEET_WRITE_URL=' "$ROOT_DIR/.env.anvil" 2>/dev/null | cut -d= -f2- || true)"
fi
DEMO_SERVICES_SHEET_URL="${VITE_DEMO_SERVICES_SHEET_URL:-${DEMO_SERVICES_SHEET_URL:-$EXISTING_SHEET_URL}}"
DEMO_SERVICES_SHEET_WRITE_URL="${VITE_DEMO_SERVICES_SHEET_WRITE_URL:-${DEMO_SERVICES_SHEET_WRITE_URL:-$EXISTING_SHEET_WRITE_URL}}"
SHEET_ENV_LINES=""
if [ -n "$DEMO_SERVICES_SHEET_URL" ]; then
  SHEET_ENV_LINES="${SHEET_ENV_LINES}
VITE_DEMO_SERVICES_SHEET_URL=$DEMO_SERVICES_SHEET_URL"
fi
if [ -n "$DEMO_SERVICES_SHEET_WRITE_URL" ]; then
  SHEET_ENV_LINES="${SHEET_ENV_LINES}
VITE_DEMO_SERVICES_SHEET_WRITE_URL=$DEMO_SERVICES_SHEET_WRITE_URL"
fi
